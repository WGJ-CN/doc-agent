...
import { getTask, listTasks, deleteTask, streamTaskProgress } from "../api.js"
...
function connectSSE(taskId) {
  ...
  closeSSE = streamTaskProgress(
    taskId,
    (data) => {
      progressSteps.value.push(data)
      if (active.value && active.value.status === "pending" && data.step !== "done") {
  ...
  <div class="m-shell">
    <div class="m-bar">
      ...
      <button
        v-if="view !== 'home'"
        class="m-bar-btn"
        @click="view === 'detail' ? goHome() : goHome()"
      >
    ...
    <div class="m-body">
      <div v-if="view === 'home'" class="m-list">
        ...
        >
          ...
          <div class="m-card-type">{{ t.doc_type }}</div>
