...
def _generate_document_text(material: str, outline: dict, doc_type: str,
    ...
    if feedback:
        ...
        system_prompt = (
            ...
            "插入 Mermaid 流程图\n"
            "4. Mermaid 图规范：graph LR 横向布局优先，subgraph 按层分组，节点六字以内，每个节点必须有连线，不放孤立节点，A -->|标注| B 箭标风格，外部节点统一定义在 subgraph 之外\n"
            "5. 接口设计章（如大纲包含）：明确接口名称、请求/响应格式、参数类型、序列化方案和异常处理\n"
            "6. 数据设计章（如大纲包含）：核心数据结构定义（字段/类型/约束）、数据字典和模块间数据流转\n"
            "7. 数据库设计章（如大纲包含）：数据库选型理由、ER 关系（Mermaid erDiagram）、"
            "核心表结构（字段名/类型/主键/外键/索引）\n"
            "8. 大纲中没有的章节一律不写，不得自行添加\n"
            "9. 所有设计描述必须与代码一致，严禁编造、猜测、虚构代码中不存在的任何设计元素\n"
            "10. 所有专业术语首次出现时给出全称\n"
            "11. 输出纯 Markdown，不要额外解释\n\n"
            f"===== 文档大纲 =====\n{sections_desc}\n===== 大纲结束 =====\n\n"
            "\n===== 搜索到的写作规范（请严格遵循） =====\n" + (json.dumps(standards_info, ensure_ascii=False, indent=2) if standards_info else "") + "\n===== 规范结束 =====\n"
            f"===== 代码资料 =====\n{mat_preview}\n===== 资料结束 ====="
        )
